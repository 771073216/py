...
class AdminPage:
    def __init__(self, parent_window):
        ...
        self.frame_left_top = Frame(width=300, height=240)  # 指定框架，在窗口上可以显示，这里指定四个框架
        self.frame_right_top = Frame(width=350, height=220)
        self.frame_center = Frame(width=800, height=200)
        ...
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中商品库存',
        ...
        self.right_top_button6 = ttk.Button(self.frame_right_top, text='查看进货单',
                                            width=20, command=lambda: GoodsAdd(self.window), padding=5)
        self.right_top_button7 = ttk.Button(self.frame_right_top, text='设置信息',
                                            width=20, command=lambda: Settings(self.window), padding=5)
        ...
        self.right_bottom_button.grid(row=0, column=1, padx=10, pady=10)  # 位置设置
        self.right_bottom_search_entry.grid(row=0, column=0, padx=10, pady=10)

        self.right_bottom_button1 = ttk.Button(self.frame_bottom, text='清除查询结果', width=20, command=self.clean_search)
        self.right_bottom_button1.grid(row=0, column=3, padx=10, pady=10)
        self.right_bottom_button2 = ttk.Button(self.frame_bottom, text='商品id查询', width=20, command=self.put_data_id)
        self.right_bottom_button2.grid(row=0, column=2, padx=10, pady=10)
        ...
        self.right_top_button5.grid(row=2, column=1, padx=5, pady=5)
        self.right_top_button6.grid(row=3, column=1, padx=5, pady=5)
        self.right_top_button7.grid(row=4, column=1, padx=5, pady=5)
    ...
    def put_data_id(self):
        self.del_button()  # 先将表格内的内容全部清空
        # 打开数据库连接，准备查找指定的信息
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        d = self.search.get()
        e = conn.cursor().execute("SELECT * FROM COMPANY WHERE id = '%s' " % d)
        results = e.fetchall()
        self.id = []
        self.name = []
        self.reminder = []
    ...
    def update_row(self):
        if self.var_id.get() == '':
            ...
            td.Daemon = True
            td.start()
            r = simpledialog.askstring(title=' ', prompt='填写进货数量')
            if not r:
                return
            self.sum = int(r) + int(self.row_info[2])
            self.cost_sum = int(r) * int(self.var_cost.get())
            # 打开数据库连接
            conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
            c = conn.cursor()
            ...
            try:
                ...
                self.tree.item(self.tree.selection()[0], values=(
                    self.var_id.get(), self.var_name.get(), self.sum,
                    self.var_model.get(), self.var_unit.get(), self.var_cost.get(), self.var_price.get()))
                self.var_reminder.set(str(self.sum))
            i = datetime.datetime.now()
            ...
            sql1 = "INSERT INTO GOODS (DATE,ID,NAME,MODEL,UNIT,COST,NUM,COST_SUM) \
                   ...
                   (date1, self.var_id.get(), self.var_name.get(), self.var_model.get(), self.var_unit.get(),
                    self.var_cost.get(), r, self.cost_sum)
            try:
    ...
    def save_sellout_excel_by_time(self):
        ...
        td.Daemon = True
        td.start()
        dd = simpledialog.askstring(title='导出记录时间', prompt='例如：导出某一年：2022   导出某一月：2022.4   导出某一天：2022.4.1')
        if not dd:
            return
    ...
    def back(self):
        NewSell(self.window)  # 进入管理员子菜单操作界面
...
class SellOut:
    def __init__(self, parent_window):
        ...
        self.frame_center = Frame(width=1200, height=200)
        # 定义下方中心列表区域
        ...
        for col in self.columns:
            self.tree.heading(col, text=col, command=lambda _col=col: self.tree_sort_column(self.tree, _col, False))
        # 整体区域定位，利用了Frame和grid进行布局
        self.frame_bottom = Frame(width=1000, height=300)
        self.search = StringVar()
        self.right_bottom_search_entry = Entry(self.frame_bottom, textvariable=self.search, font=('微软雅黑', 15))
        ...
        self.right_bottom_button5 = ttk.Button(self.frame_bottom, text='选中商品退货',
                                               width=20, command=self.goods_rollback, padding=5)
        self.right_bottom_button5.grid(row=3, column=1, padx=0, pady=0)
    ...
    def goods_rollback(self):
        self.rollback = simpledialog.askinteger(title=' ', prompt='退货数量：')
        if not self.rollback:
            return
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        c = conn.cursor()
        e = c.execute(
            "SELECT * FROM SALE WHERE date = '%s' and id = '%s'" % (self.goods_date, self.goods_id)).fetchall()
        results = e[0]
        after_num = int(results[7]) - int(self.rollback)
        if after_num == 0:
            sql = "DELETE from SALE where DATE='%s' and ID='%s'" % (self.row_info[0], self.row_info[1])
            try:
                c.execute(sql)
                conn.commit()
            except (sqlite3.ProgrammingError, sqlite3.OperationalError):
            ...
            messagebox.showinfo('警告！', '更新失败，退货数量不能大于售出数量！')
            return
        after_cost_sum = after_num * int(self.goods_cost)
        after_price_sum = after_num * int(self.goods_price)
        after_get = after_price_sum - after_cost_sum
        try:
            c.execute("UPDATE SALE set NUM='%s', COST_SUM='%s',SUM='%s',GET='%s' where DATE = '%s' and ID = '%s'" %
                      (after_num, after_cost_sum, after_price_sum, after_get, self.goods_date, self.goods_id))
            conn.commit()
            goods_reminder = c.execute("SELECT REMINDER from COMPANY  where ID='%s'" % self.goods_id).fetchall()
            goods_reminder1 = goods_reminder[0]
            goods_reminder2 = int(goods_reminder1[0]) + int(self.rollback)
            sql = "UPDATE COMPANY set REMINDER = '%s' where ID='%s'" % \
                  (goods_reminder2, self.goods_id)
            c.execute(sql)
            conn.commit()
        except (sqlite3.ProgrammingError, sqlite3.OperationalError):
            conn.rollback()
    ...
    def click(self, event):
        ...
        self.row = self.tree.identify_row(event.y)  # 行
        self.row_info = self.tree.item(self.row, "values")
        self.goods_date = self.row_info[0]
        self.goods_id = self.row_info[1]
        self.goods_cost = self.row_info[5]
        self.goods_price = self.row_info[6]
        self.goods_num = self.row_info[7]
...
class NewSell:
    def __init__(self, parent_window):
        ...
        self.window.title('管理面板')
        self.window.geometry("800x570+300+30")  # 初始窗口在屏幕中的位置
        self.frame_left_top = Frame(width=800, height=200)  # 指定框架，在窗口上可以显示，这里指定四个框架
        self.frame_right_top = Frame(width=800, height=50)
        self.frame_center = Frame(width=800, height=200)
        self.frame_bottom = Frame(width=800, height=50)
        self.id = []
        self.name = []
        ...
        self.columns1 = ("商品id", "名称", "余货量", "型号", "单位", "单价")
        ...
        self.tree1.column("型号", width=150, anchor='center')
        ...
        for i in range(min(len(self.id1), len(self.name1), len(self.reminder1),
                           len(self.model1), len(self.unit1), len(self.price1))):  # 写入数据
            self.tree1.insert('', i, values=(self.id1[i], self.name1[i], self.reminder1[i],
                                             self.model1[i], self.unit1[i], self.price1[i]))
        ...
        self.right_top_button7 = ttk.Button(self.frame_right_top, text='管理面板',
                                            width=20, command=lambda: AdminPage(self.window), padding=5)
        ...
        self.right_top_button7.grid(row=2, column=3, padx=20, pady=0)
    ...
    def push_sell(self):
        ...
        for var in [self.name, self.model, self.unit, self.num, self.price, self.sum]:
            ...
            n = n + 1
            for i in range(len(m)):
                ws.cell(i + 4, n + 1).value = str(m[i])
                ws.cell(i + 4, n + 1).border = border_set
        n = 0
        for i in range(len(m)):
            n = n + 1
            ws.cell(i + 4, 1).value = str(n)
    ...
    def click1(self, event):
        ...
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        c = conn.cursor()
        sql = "SELECT * from COMPANY where ID='%s'" % (self.row_info1[0])
        cursor = c.execute(sql)
        results = cursor.fetchall()
        self.click_get = results[0]
        self.id11 = self.var_id1.get()
        self.var_name1.set(self.click_get[1])
        self.var_reminder1.set(self.click_get[2])
        self.var_model1.set(self.click_get[3])
        self.var_unit1.set(self.click_get[4])
        self.var_cost1.set(self.click_get[5])
        self.var_price1.set(self.click_get[6])
    ...
    def sellout(self):
        uuu = int(self.click_get[6])
        price = uuu * self.result
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        c = conn.cursor()
        sql = "INSERT INTO SELL (ID,NAME,MODEL,UNIT,NUM,PRICE,SUM) \
                                           VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')" % \
              (self.row_info1[0], self.click_get[1],
               self.click_get[3], self.click_get[4], self.result, self.click_get[6], price)
        try:
            c.execute(sql)  # 执行sql语句
            conn.commit()  # 提交到数据库执行
    ...
    def edit_num(self):
        ...
        td.Daemon = True
        td.start()
        self.result = simpledialog.askinteger(title=' ', prompt='售出数量：')
        if not self.result:
            return
        self.id11 = self.var_id1.get()
        d = self.result
        ...
        if k >= 0:
            ...
            try:
                ...
                self.tree1.item(self.tree1.selection()[0], values=(
                    ...
                    self.var_model1.get(), self.var_unit1.get(), self.var_price1.get()))
                k1 = str(k)
...
class GoodsAdd:
    def __init__(self, parent_window):
        ...
        self.frame_center = Frame(width=1200, height=200)
        # 定义下方中心列表区域
        self.columns = ('日期', '商品id', '名称', '型号', '单位', '单价', '数量', '总价')
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=8, columns=self.columns)
        # 添加竖直滚动条
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)
        # 定义树形结构与滚动条
        ...
        self.tree.column("型号", width=100, anchor='center')
        self.tree.column("单位", width=50, anchor='center')
        self.tree.column("单价", width=50, anchor='center')
        ...
        for row in results:
            ...
            self.cost_sum.append(row[7])
        conn.close()
        # 写入数据
        for i in range(min(len(self.date), len(self.id), len(self.name), len(self.model),
                           len(self.unit), len(self.cost), len(self.num), len(self.cost_sum))):
            self.tree.insert('', i, values=(self.date[i], self.id[i], self.name[i], self.model[i],
                                            self.unit[i], self.cost[i], self.num[i], self.cost_sum[i]))
        # 绑定函数，使表头可排序
        for col in self.columns:
            self.tree.heading(col, text=col, command=lambda _col=col: self.tree_sort_column(self.tree, _col, False))
        # 整体区域定位，利用了Frame和grid进行布局
        ...
        self.right_bottom_button = ttk.Button(self.frame_bottom, text='按名称查询',
                                              width=20, command=self.put_data, padding=5)
        self.right_bottom_button.grid(row=1, column=0, padx=0, pady=0)  # 位置设置
        self.right_bottom_search_entry.grid(row=0, column=0, padx=0, pady=20)
        self.right_bottom_title = Label(self.frame_bottom, text="按时间查询，例如：2022.4.3", font=('Verdana', 15))
        self.right_bottom_title.grid(row=2, column=1, pady=10)
        self.right_bottom_button1 = ttk.Button(self.frame_bottom, text='清除查询结果',
                                               width=20, command=self.clean_search, padding=5)
        self.right_bottom_button1.grid(row=0, column=1, padx=20, pady=20)
        self.right_bottom_button2 = ttk.Button(self.frame_bottom, text='按时间查询',
                                               width=20, command=self.put_data_time, padding=5)
        self.right_bottom_button2.grid(row=2, column=0, padx=20, pady=20)
        self.right_bottom_button4 = ttk.Button(self.frame_bottom, text='删除选中商品进货信息',
                                               width=20, command=self.del_row, padding=5)
        self.right_bottom_button4.grid(row=3, column=0, padx=20, pady=20)
    ...
    def del_row(self):
        if not self.row_info:
            ...
            if res:
                ...
                conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
                c = conn.cursor()
                sql = "DELETE from GOODS where DATE='%s'" % (self.row_info[0])
                try:
                    c.execute(sql)
                    conn.commit()
                except (sqlite3.ProgrammingError, sqlite3.OperationalError):
    ...
    def put_data(self):
        ...
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        d = self.search.get()
        e = conn.cursor().execute("SELECT * FROM GOODS WHERE name like '%" + d + "%'")
        results = e.fetchall()
        for row in results:
            self.date.append(row[0])
            self.id.append(row[1])
    ...
    def put_data_time(self):
        ...
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        d = self.search.get().replace('.', '/')
        e = conn.cursor().execute("SELECT * FROM GOODS WHERE date like '%" + d + "%'")
        results = e.fetchall()
        for row in results:
            self.date.append(row[0])
            self.id.append(row[1])
...
class Settings:
    def __init__(self, parent_window):
        ...
        self.window = Tk()  # 初始框的声明
        self.window.title('管理面板')
        self.window.geometry("400x200+500+200")  # 初始窗口在屏幕中的位置
        self.frame_center = Frame(width=400, height=200)
        self.name = StringVar()
        self.locale = StringVar()
        self.number = StringVar()
        self.right_top_model_label = Label(self.frame_center, text="清单名称：", font=('微软雅黑', 15))
        self.right_top_model_entry = Entry(self.frame_center, textvariable=self.name, font=('微软雅黑', 15))
        self.right_top_model_label.grid(row=1, column=0, padx=5, pady=5)  # 位置设置
        self.right_top_model_entry.grid(row=1, column=1, padx=5, pady=5)

        self.right_top_unit_label = Label(self.frame_center, text="地点：", font=('微软雅黑', 15))
        self.right_top_unit_entry = Entry(self.frame_center, textvariable=self.locale, font=('微软雅黑', 15))
        self.right_top_unit_label.grid(row=2, column=0, padx=5, pady=5)  # 位置设置
        self.right_top_unit_entry.grid(row=2, column=1, padx=5, pady=5)

        self.right_top_cost_label = Label(self.frame_center, text="电话：", font=('微软雅黑', 15))
        self.right_top_cost_entry = Entry(self.frame_center, textvariable=self.number, font=('微软雅黑', 15))
        self.right_top_cost_label.grid(row=3, column=0, padx=5, pady=5)  # 位置设置
        self.right_top_cost_entry.grid(row=3, column=1, padx=5, pady=5)

        self.right_top_button1 = ttk.Button(self.frame_center, text='修改信息',
                                            width=20, command=self.edit_info, padding=5)
        self.right_top_button1.grid(row=4, column=1, padx=5, pady=20)
        self.frame_center.grid(row=1, column=0, columnspan=2, padx=5, pady=5)
        conn = sqlite3.connect(os.path.expanduser("~/sql/list.db"))
        c = conn.cursor()
        try:
            ...
            sql = "INSERT INTO INFO (NAME,LOCALE,NUMBER) VALUES ('%s', '%s', '%s')" % \
                  ('', '', '')
            c.execute(sql)
            conn.commit()
        else:
            self.name.set(info[0])
            self.locale.set(info[1])
            self.number.set(info[2])
        conn.close()
    ...
    def edit_info(self):
        ...
        sql = "UPDATE INFO set NAME = '%s',LOCALE = '%s',NUMBER = '%s'" % \
              (self.name.get(), self.locale.get(), self.number.get())
        try:
            c.execute(sql)
            conn.commit()
        except (sqlite3.ProgrammingError, sqlite3.OperationalError):
