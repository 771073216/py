...
from PyQt5.QtWidgets import QMessageBox, QInputDialog, QHeaderView, QFileDialog
...
class FirstWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.table_goods.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table_goods.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.table_goods.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
        self.table_goods.cellPressed.connect(self.get_click)
        self.table_goods.verticalHeader().setVisible(False)

        self.table_new_goods.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

        self.table_sale.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table_sale.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.table_sale.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        self.table_sale.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.table_sale.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
        self.table_sale.horizontalHeader().setSectionResizeMode(7, QHeaderView.ResizeToContents)
        ...
        self.table_stock.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.table_stock.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        self.table_stock.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.table_stock.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
        self.table_stock.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeToContents)
        self.table_stock.verticalHeader().setVisible(False)
        self.table_stock.cellPressed.connect(self.get_stock_click)

        self.table_prepare_goods.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table_prepare_goods.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.table_prepare_goods.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
        ...
        self.table_sell_out.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table_sell_out.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.table_sell_out.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
    ...
    def merge_new_goods_table(self):
        ...
        for i in err:
            k = k + 1
            if i == 'id_err':
                QMessageBox.warning(self, '警告！', '合并失败，第 ' + str(k) + ' 行商品id冲突！', QMessageBox.Yes)
            else:
                QMessageBox.warning(self, '警告！', '合并失败，第 ' + str(k) + ' 行信息有错误！', QMessageBox.Yes)

        if merge_flag == 1 and error_flag == 1:
            QMessageBox.information(self, '提示！', '正确的数据合并成功！', QMessageBox.Yes)
    ...
    def del_goods_row(self):
        if not self.row1 and self.row1 != 0:
            QMessageBox.warning(self, '提示！', '请点击要删除的项目！', QMessageBox.Yes)
        else:
            reply = QMessageBox.warning(self, '警告', '是否删除所选数据？', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                ...
                try:
                    ...
                    conn.rollback()
                    QMessageBox.warning(self, '提示', '删除失败，数据库删除操作失败！', QMessageBox.Yes)
                else:
                    conn.close()
    ...
    def update_goods_reminder(self):
        if not self.row1 and self.row1 != 0:
            QMessageBox.warning(self, '警告！', '请点击要更新的项目！', QMessageBox.Yes)
        else:
            ...
            num, ok = QInputDialog.getInt(self, '输入框', '输入数字')
            if ok is False:
                self.row1 = None
                return
            elif ok is True and num <= 0:
                QMessageBox.information(self, '提示！', '请输入一个大于 0 的数！', QMessageBox.Yes)
            else:
                ...
                try:
                    ...
                    conn.rollback()
                    QMessageBox.warning(self, '警告！', '更新失败，数据库写入操作失败！', QMessageBox.Yes)
                else:
    ...
    def sale_rollback(self):
        if not self.row1 and self.row1 != 0:
            QMessageBox.warning(self, '提示！', '请点击要退货的项目！', QMessageBox.Yes)
        else:
            num, ok = QInputDialog.getInt(self, '退货数量', '输入数字')
            if ok is False:
                self.row1 = None
                return
            elif ok is True and num <= 0:
                ...
                if after_num == 0:
                    ...
                    try:
                        ...
                        conn.rollback()
                        QMessageBox.warning(self, '警告！', '删除失败，数据库删除操作失败！', QMessageBox.Yes)
                    else:
                        self.table_sale.removeRow(self.table_sale.currentRow())
                elif after_num < 0:
                    QMessageBox.warning(self, '警告！', '更新失败，退货数量不能大于售出数量！', QMessageBox.Yes)
                    return
                else:
        ...
        if not self.row1 and self.row1 != 0:
            ...
            if ok is False:
                ...
                try:
                    ...
                    conn.rollback()
                    QMessageBox.warning(self, '警告！', '更新失败，请勿重复出售相同的商品！', QMessageBox.Yes)
                except (sqlite3.ProgrammingError, sqlite3.OperationalError):
                    conn.rollback()
                    QMessageBox.warning(self, '警告！', '更新失败，数据库写入操作失败！', QMessageBox.Yes)
                else:
    ...
    def sell_out(self):
        ...
        if name == '' or locale == '' or number == '':
            QMessageBox.information(self, '提示！', '未填写Excel信息！', QMessageBox.Yes)
        ...
        try:
            ...
            QMessageBox.warning(self, '警告！', '导出失败，请检查文件是否被占用！', QMessageBox.Yes)
        else:
            self.put_sale()
            self.clean_sale()
    ...
    def put_sale(self):
        ...
        for i in range(len(result1)):
            ...
            try:
                ...
                conn.rollback()
                conn.close()
                QMessageBox.warning(self, '警告！', '数据库写入操作失败！', QMessageBox.Yes)
            else:
    ...
    def save_sellout_excel_by_time(self):
        ...
        if len(date) == 0:
            QMessageBox.warning(self, '警告！', '请输入导出记录的时间！', QMessageBox.Yes)
            return
        elif len(date) == 1:
            d = date[0]
        elif len(date) == 2:
            ...
            QMessageBox.warning(self, '警告！', '请输入正确的时间！', QMessageBox.Yes)
            return
